...
public:
    ...
    void register_var(string name, Type type, bool _push = 1)
    {
        get_type[++num_var] = type;
        if(_push) get_var[name].push(num_var);
    }
    ...
    int check_initval(Node* o, int L)
    {
        ...
        if (!o->child.size())
        {
            ...
            // cerr << "LINE : " << __LINE__ << " " << type.wid.size() << "\n";
            o->exp_type = type;
        }
        else if (o->child[0]->type == "InitVals")
        {
            ...
            // cerr << "LINE : " << __LINE__ << " " << o->exp_type.wid.size() << "\n";
        }
        else
        {
            if(!check(o->child[0], L)) { DEBUG("type error"); return 0;}
    ...
    int check_Block(Node* o, int L, bool modify = 1)
    {
        ...
        {
            if(t.size() && abs(t.top()) > L && !get_type[t.top()].isfunc) 
            {
                // cerr << "here I pop from " << _ << " and the val is " << t.top() << "\n"; 
                t.pop();
            }
        }
        return 1;
    ...
    int check_vardecl(Node* o, int L)
    {
        ...
        for(int i = 1; i < o->child.size(); ++i)
        {
            ...
            else if (get_var[ident->text].size() && abs(get_var[ident->text].top()) > L) { DEBUG("type error"); return 0; }
            
            if (L == 0) ident->val = -num_var-1;
            else ident->val = num_var + 1;

            // cerr << "HELLO !!! " << L << " " << ident->text << " " << ident->val << "\n";
            // cerr << "I want to push : " << get_var[ident->text].size() << "\n";
            // if(get_var[ident->text].size() != 0) cerr << " " << get_var[ident->text].top() << "\n";
            get_var[ident->text].push(ident->val);
            // cerr << get_var[ident->text].top() << " look here is ok!\n";
            ...
            // std::cerr << "all = " << all << "\n";
            register_var(ident->text, type, 0);
    ...
    int check_lval(Node* o, int L)
    {
        ...
        if (!get_var.count(ident->text) || !get_var[ident->text].size()) { 
            // std::cerr << ident->text << " : Not found!\n";
            DEBUG("type error"); return 0;
        }
        int id = get_var[ident->text].top();
        if(ident->val == 0) ident->val = id;
        // cerr << "yet I FOUND " << ident->text << " " << id << "\n";
        Type type = get_type[abs(id)];
        if (o->child.size() > 1)
        {
            Node *arr_wid = o->child[1];
            // cerr << type.wid.size() << " " << arr_wid->child.size() << "\n";
    ...
    int check_funcdef(Node* o, int L)
    {
        ...
        if(ident->val == 0) ident->val = num_var+1;
    ...
    int check_funcparam(Node* o, int L)
    {
        ...
        {
            get_var[ident->text] = stack<int>();
            while(get_var[ident->text].size()) get_var[ident->text].pop();
        }
        else if (get_var[ident->text].size() && abs(get_var[ident->text].top()) > L) { DEBUG("type error"); return 0;}
        ...
        if(ident->val == 0) ident->val = num_var;
        return 1;
    }
    ...
    {
        Node *ident = o->child[0];
        if(!get_var.count(ident->text) || get_var[ident->text].size() == 0) 
            { DEBUG("type error"); return 0;}
        int id = get_var[ident->text].top();
        if(ident->val == 0) ident->val = id;
